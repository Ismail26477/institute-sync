  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">User Roles & Access</h1>
          <p className="text-sm text-muted-foreground">Role-based access control (RBAC) management</p>
        </div>
        <button className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity"><Plus className="w-4 h-4" /> Add User</button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="kpi-card flex items-center gap-3"><Users className="w-8 h-8 text-info" /><div><p className="text-xl font-display font-bold text-foreground">{usersData.length}</p><p className="text-sm text-muted-foreground">Admin Users</p></div></div>
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="bg-card rounded-xl border border-border/50 shadow-card p-6">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h2 className="text-xl font-display font-bold text-foreground">{selectedRole.name}</h2>
            ...
            <div className="flex flex-wrap gap-2">
              {selectedRole.permissions.map((p) => (
                <span key={p} className="inline-flex items-center gap-1 text-xs px-3 py-1 rounded-full bg-success/10 text-success font-medium"><Check className="w-3 h-3" />{p}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {activeTab === "Users" && (
        <div className="bg-card rounded-xl border border-border/50 shadow-card overflow-hidden animate-fade-in">
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/30">
          ...
          </tr></thead><tbody>{usersData.map((u) => (
            <tr key={u.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
              ...
              <td className="py-3 px-4 text-right"><div className="flex items-center justify-end gap-1">
                ...
                <button className="p-1.5 rounded-md hover:bg-muted transition-colors text-destructive"><Trash2 className="w-4 h-4" /></button>
              </div></td>
            </tr>
          ))}</tbody></table></div>
        </div>
