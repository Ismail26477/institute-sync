...
import { Search, Plus, BookOpen, RotateCcw, AlertTriangle, Users, Barcode } from "lucide-react";
...
export default function LibraryPage() {
  ...
  const filteredCatalog = catalogData.filter(b =>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card text-sm font-medium text-foreground hover:bg-muted transition-colors"><Barcode className="w-4 h-4" /> Scan</button>
          <button className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity"><Plus className="w-4 h-4" /> Add Book</button>
        </div>
      </div>

      {/* KPI */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="kpi-card flex items-center gap-3"><BookOpen className="w-8 h-8 text-primary" /><div><p className="text-xl font-display font-bold text-foreground">8,450</p><p className="text-sm text-muted-foreground">Total Books</p></div></div>
        <div className="kpi-card flex items-center gap-3"><Users className="w-8 h-8 text-info" /><div><p className="text-xl font-display font-bold text-foreground">342</p><p className="text-sm text-muted-foreground">Books Issued</p></div></div>
      ...
      {activeTab === "Circulation" && (
        <div className="animate-fade-in bg-card rounded-xl border border-border/50 shadow-card overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/30">
          ...
          </tr></thead><tbody>{circulationData.map((c) => (
            <tr key={c.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
              ...
              <td className="py-3 px-4 text-right"><button className="text-xs text-primary font-medium hover:underline">{c.status === "overdue" ? "Send Reminder" : "Return"}</button></td>
            </tr>
          ))}</tbody></table></div>
        </div>
      )}
        ...
        <div className="animate-fade-in bg-card rounded-xl border border-border/50 shadow-card overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/30">
            ...
            <tr key={o.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
              ...
              <td className="py-3 px-4 text-right"><button className="text-xs text-primary font-medium hover:underline">Send Reminder</button></td>
            </tr>
          ))}</tbody></table></div>
        </div>
      )}
      ...
      {activeTab === "Inventory" && (
        <div className="animate-fade-in space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="kpi-card"><p className="text-xl font-display font-bold text-foreground">8,450</p><p className="text-sm text-muted-foreground">Total Volumes</p></div>
            <div className="kpi-card"><p className="text-xl font-display font-bold text-foreground">1,245</p><p className="text-sm text-muted-foreground">Unique Titles</p></div>
