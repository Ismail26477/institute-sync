...
export default function InstitutesPage() {
  ...
  const [selectedInstitute, setSelectedInstitute] = useState<typeof institutesData[0] | null>(null);
  ...
  const totalStudents = institutesData.reduce((a, b) => a + b.students, 0);
  const totalFaculty = institutesData.reduce((a, b) => a + b.faculty, 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{institutesData.length} institutes · {totalStudents} students · {totalFaculty} faculty</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-border bg-card text-sm text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-ring outline-none"
          />
        </div>
      </div>
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="bg-card rounded-xl border border-border/50 shadow-card p-6">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-display font-bold text-foreground">{selectedInstitute.name}</h2>
                <p className="text-sm text-muted-foreground">Code: {selectedInstitute.code} · Est. {selectedInstitute.established}</p>
              </div>
            ...
            <div key={inst.id} className="bg-card rounded-xl border border-border/50 shadow-card p-5 hover:shadow-elevated transition-all animate-fade-in">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-display font-semibold text-foreground truncate">{inst.name}</h3>
                  <p className="text-xs text-muted-foreground">{inst.code} · Est. {inst.established}</p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="text-center p-2 rounded-lg bg-muted/50">
                  <p className="text-lg font-bold text-foreground">{inst.students}</p>
                  <p className="text-[10px] text-muted-foreground">Students</p>
                </div>
                <div className="text-center p-2 rounded-lg bg-muted/50">
                  <p className="text-lg font-bold text-foreground">{inst.faculty}</p>
                  <p className="text-[10px] text-muted-foreground">Faculty</p>
                </div>
                <div className="text-center p-2 rounded-lg bg-muted/50">
                  <p className="text-lg font-bold text-foreground">{inst.programs}</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setSelectedInstitute(inst)} className="inline-flex items-center gap-1 text-xs text-primary font-medium hover:underline">
