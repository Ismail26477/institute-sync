...
import { IndianRupee, Search, Plus, Download, Eye, Send, FileText, Filter } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">Fees & Billing</h1>
          <p className="text-sm text-muted-foreground">Manage invoices, fee templates, and scholarships</p>
        </div>
        <div className="flex gap-2">
          ...
          <button className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity"><Plus className="w-4 h-4" /> Create Invoice</button>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-muted rounded-lg p-1 w-fit">
        {tabs.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === tab ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {activeTab === "Invoices" && (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div className="kpi-card"><p className="text-xl font-display font-bold text-foreground">₹6,15,000</p><p className="text-sm text-muted-foreground">Total Invoiced</p></div>
            <div className="kpi-card"><p className="text-xl font-display font-bold text-success">₹4,08,000</p><p className="text-sm text-muted-foreground">Collected</p></div>
            <div className="kpi-card"><p className="text-xl font-display font-bold text-warning">₹68,000</p><p className="text-sm text-muted-foreground">Partially Paid</p></div>
            <div className="kpi-card"><p className="text-xl font-display font-bold text-destructive">₹1,39,000</p><p className="text-sm text-muted-foreground">Overdue</p></div>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border/50 shadow-card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{invoicesData.filter(i => i.student.toLowerCase().includes(search.toLowerCase()) || i.id.toLowerCase().includes(search.toLowerCase())).map((inv) => (
            ...
            {scholarships.map((s) => (
              <div key={s.id} className="bg-card rounded-xl border border-border/50 shadow-card p-5 hover:shadow-elevated transition-all">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${statusStyle[s.status]}`}>{s.status}</span>
                </div>
